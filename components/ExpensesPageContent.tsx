...
export default function ExpensesPageContent({ scope }: ExpensesPageContentProps) {
  ...
  const exportToCSV = () => {
    ...
    const filterParts: string[] = [scope];
    ...
    const filename = `expenses${filterSuffix}-${format(new Date(), 'yyyy-MM-dd')}.csv`;
  ...
  const handleMoveScope = async (transactionIds: string[], targetShared: boolean) => {
    try {
      const response = await fetch('/api/transactions/bulk-update', {
        ...
        body: JSON.stringify({
          ...
          updates: { is_shared: targetShared },
        }),
      });

      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Failed to move expenses');
      }

      setSelectedTransactionIds(new Set());
      loadTransactions();
    } catch (error: any) {
      alert(error.message || 'Failed to move expenses');
    }
  };
  ...
  const matchesCurrentFilters = useCallback(
    ...
    [categories, selectedCategories, selectedPeriod, selectedPaidBy, selectedPaymentMethod, selectedYear]
  );
    ...
    <main className={`min-h-screen p-1 sm:p-4 lg:p-8 bg-gray-50 ${selectedTransactionIds.size > 0 ? 'pb-32' : ''}`}>
      <div className="max-w-7xl mx-auto">
        ...
        {showCSVImport && (
          <div className="mb-8 p-6 bg-white border rounded-lg">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-semibold">Import Expenses from CSV</h2>
              <button
                onClick={() => {
        ...
        {showScreenshotImport && (
          <div className="mb-8 p-6 bg-white border rounded-lg">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-semibold">Import Expenses from Screenshot</h2>
              <button
                onClick={() => {
               ...
               <TransactionList
                 ...
                 onAddTransaction={async (data) => {
                   const response = await fetch('/api/transactions', {
                     ...
                     headers: { 'Content-Type': 'application/json' },
                     credentials: 'include',
                     body: JSON.stringify({ ...data, is_shared: isShared }),
                   });
        ...
        {selectedTransactionIds.size > 0 && (
          <BulkEditBar
            ...
            onBulkDelete={async (ids: string[]) => {
              ...
              if (failed.length > 0) {
                throw new Error(`Failed to delete ${failed.length} expense${failed.length > 1 ? 's' : ''}`);
              }
            ...
            extraActions={
              <button
                onClick={() => handleMoveScope(Array.from(selectedTransactionIds), !isShared)}
              ...
              >
                Move to {isShared ? 'Personal' : 'Shared'}
              </button>
            }
          />
        )}
